...
- 滑杆微调，并支持在预览上鼠标/单指拖动、滚轮/双指缩放
- 动态限制缩放与平移，确保照片始终覆盖输出画布
- 写入 300DPI `pHYs` 元数据的单张 PNG 下载
- 4R（4×6in）和 A4 300DPI 自动多图拼版，并写入打印分辨率元数据
- 可选裁切线及拼版 PNG 下载
- 中文响应式界面
...
- [证件水印](https://watermark.i41.cn)：敏感证件用途水印，纯前端本地处理
- [临时剪贴板](https://clip.i41.cn)：客户端加密、自动过期和阅后即焚的跨端文本传递
...
照片与抠图均在浏览器本地处理，应用本身不设上传接口。首次使用抠图功能时会向 IMG.LY CDN 请求运行资源，但照片不会随该请求发送。

页面加载 i41 匿名统计，用于发送匿名访问、性能、UTM 归因及跨站点击数据；不会收集或发送照片、文件名、导出内容或永久标识。Cloudflare Pages 仍可能注入其 Web Analytics Beacon。
